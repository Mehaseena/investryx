...
class _AddAdvisorProfileScreenState extends State<AddAdvisorProfileScreen> {
  ...
  final _areaOfInterestController = TextEditingController(text: '0 Years');
  ...
  bool _validateForm() {
    ...
    // Make sure experience and area of interest are not empty or default value
    if (_yearExperienceController.text.isEmpty || _yearExperienceController.text == '0 Years') {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
    ...
    if (_areaOfInterestController.text.isEmpty || _areaOfInterestController.text == '0 Years') {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
      ...
      try {
        ...
        if (mounted) {
          if (response == true) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Advisor profile created successfully!'),
                backgroundColor: Colors.green,
              ),
            );
            Navigator.push(context, MaterialPageRoute(builder: (context)=>DigiLockerVerificationScreen(profileImage: _imageFile,)));
          } else if (response == false) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Failed to create advisor profile. Please try again.'),
                backgroundColor: Colors.red,
              ),
            );
          } else {
  ...
  bool _formHasData() {
    return _fullNameController.text.isNotEmpty ||
        ...
        _areaOfInterestController.text != '0 Years' ||
